...
const CompleteProfile = () => {
  ...
  useEffect(() => {
    console.log("inside set initial form values useEffect");
    console.log("values: ", values);
    if (Object.entries(values).length === 0) {
      console.log("got empty values state obj");
      let fields = {
        firstName: "",
        lastName: "",
        contact: "",
  ...
  function sendForm(e) {
    console.log("got sendForm!", values);
    ...
    axios({
      ...
      .then((res) => {
        ...
        if (res.data.status == "Success") {
          // setSnack(true);
          notifySuccess();
          document.cookie = "token=; 01 Jan 1970 00:00:00 UTC; path=/;";
          setTimeout(function () {
            history.push("/");
          }, 5200);
        } else {
  ...
  const handleImageChange = (e) => {
    if (e.target.files.length) {
      ...
      if (e.target.files[0].size > 700000) {
        console.log("got image too large");
        notify();
      } else {
        setImage({
    ...
    <section className={`l-content c-createForm`}>
      ...
      <form id="c-createForm" onSubmit={handleSubmit}>
        ...
        <div className={`l-content__flexContainer--right u-mt15`}>
          ...
          <button className="e-btn--small" type="submit">
